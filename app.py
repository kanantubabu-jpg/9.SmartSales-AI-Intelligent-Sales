...
st.markdown(
    'A polished dashboard for customer segmentation, predictive analytics, and intelligent product recommendations.'
)
st.divider()
...
with st.sidebar:
    ...
    st.caption('Refine customer cohorts, explore segments, and manage dataset records from one unified panel.')
    ...
    st.markdown('---')
    st.markdown('### Upcoming improvements')
    st.write(
        '- Enhanced forecasting and customer lifetime value predictions\n'
        '- Automated PDF insights and data extraction\n'
        '- Smart report exports and real-time alerting'
    )
...
# Layout tabs
tabs = st.tabs(['Explorer', 'Profile & CRUD', 'Add Customer', 'Analytics', 'Prediction', 'Income Analysis', 'Product Catalog', 'Upload & Analyze'])
...
with tabs[7]:
    st.subheader('Upload and analyze your dataset')
    st.write('Upload a CSV to run a quick analysis and generate predictions from the existing customer models. Upload a PDF to extract and preview text content.')

    uploaded_file = st.file_uploader('Choose a CSV or PDF file', type=['csv', 'pdf'], accept_multiple_files=False)
    if uploaded_file is not None:
        if uploaded_file.type == 'text/csv' or uploaded_file.name.lower().endswith('.csv'):
            uploaded_df = process_csv_upload(uploaded_file)
            if uploaded_df is None:
                st.error('Unable to parse the CSV file. Please check the formatting and try again.')
            else:
                st.success('CSV file loaded successfully.')
                st.markdown('### Uploaded data preview')
                st.dataframe(uploaded_df.head(10))
                st.write('#### Dataset summary')
                st.write(uploaded_df.describe(include='all').transpose())

                required_columns = {'age', 'salary', 'monthly_expenditures'}
                if required_columns.issubset(uploaded_df.columns):
                    preview_df = uploaded_df.copy()
                    preview_df = preview_df.dropna(subset=['age', 'salary', 'monthly_expenditures'])
                    preview_df['predicted_transactions'] = reg_model.predict(preview_df[['age', 'salary', 'monthly_expenditures']])
                    preview_df['churn_risk'] = churn_model.predict(preview_df[['age', 'salary', 'monthly_expenditures']])
                    preview_df['churn_risk'] = preview_df['churn_risk'].map({0: 'Low', 1: 'High'})

                    st.markdown('### Prediction results')
                    st.metric('Predicted avg. monthly transactions', f"{preview_df['predicted_transactions'].mean():.1f}")
                    churn_counts = preview_df['churn_risk'].value_counts().to_dict()
                    st.metric('High churn risk profiles', churn_counts.get('High', 0))
                    st.metric('Low churn risk profiles', churn_counts.get('Low', 0))

                    st.write('### Prediction preview')
                    st.dataframe(preview_df[['age', 'salary', 'monthly_expenditures', 'predicted_transactions', 'churn_risk']].head(10))
                    ...
                    fig_upload = px.scatter(
                        ...
                        color='churn_risk',
                        size='predicted_transactions',
                        title='Uploaded Data: Salary vs Expenditure with Predictions',
                        labels={'salary': 'Salary', 'monthly_expenditures': 'Monthly Expenditure', 'predicted_transactions': 'Predicted Transactions'}
                    )
                    st.plotly_chart(fig_upload, use_container_width=True)
                else:
                    st.warning('Uploaded CSV does not contain the required numeric columns for prediction: age, salary, monthly_expenditures.')
                    st.write('Available columns:', uploaded_df.columns.tolist())
        elif uploaded_file.type == 'application/pdf' or uploaded_file.name.lower().endswith('.pdf'):
            text = extract_text_from_pdf(uploaded_file)
            if text is None or len(text.strip()) == 0:
                st.error('Unable to extract text from the PDF file. Please upload a different PDF or check file contents.')
            else:
                st.success('PDF content extracted successfully.')
                st.markdown('### PDF text preview')
                st.text_area('Extracted text', text[:10000], height=320)
                st.write('Use this preview to confirm the document contents before future data extraction and model ingestion.')
        else:
